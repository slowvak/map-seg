...
class TestLoadNifti:
    def test_valid_file(self, tmp_path):
        ...
        img, min_val, max_val = load_nifti(path)
...
class TestGetMapping:
    ...
    def test_nonzero_min(self, monkeypatch):
        inputs = iter(["10", "20"])
        monkeypatch.setattr("builtins.input", lambda _: next(inputs))
        mapping = get_mapping(3, 4)
        assert mapping == {3: 10, 4: 20}
...
class TestEndToEnd:
    def test_round_trip(self, tmp_path, monkeypatch):
        ...
        img, min_val, max_val = load_nifti(input_path)
